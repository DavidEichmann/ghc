...
def check_stats(test, metric, way, expected_val, actual_val, tolerance_dev, force_print = False):
    ...
    if actual_val < lowerBound:
        ...
        return failBecause('stat too good', tag='stat')
    if actual_val > upperBound:
        print(metric, 'value is too high:')
        return failBecause('stat not good enough', tag='stat')

    if actual_val < lowerBound or actual_val > upperBound or force_print:
