...
# parse_perf_stat :: String -> PerfStat
def parse_perf_stat(stat_str):
    field_vals = stat_str.strip('\t').split('\t')
    return PerfStat(*field_vals)

# Get the expected performance stat value for a given test, metric, and commit.
def get_expected_perf_stat(test_name, metric, commit="HEAD"):
    
    # Earliest metric value (avg within commit note) since and including latest allowed change commit
    
    note_hash_to_object_hash = _git_notes_list('perf')

    # Get all ancestor notes.
    valid_note_hashes = [note_hash for (note_hash, obj_hash) in note_hash_to_object_hash.items()
                            if _git_is_ancestor(obj_hash, commit)]

# Returns True if a is an ancestor of b (or if a is b). False otherwise.
def _git_is_ancestor(a, b):
    return subprocess.call(['git', 'merge-base', '--is-ancestor', a, b]) == 0
    
# Get a dictionary from git note hash to their attached git object hash
def _git_notes_list(namespace='perf'):
    notes_str = subprocess.check_output(['git', 'notes', '--ref=' + namespace, 'list'], stderr=subprocess.STDOUT).decode('utf-8')
    return dict(re.findall('^(\w+) (\w+)', notes_str))
...
# Check if a change is allowed for a test.
# allowed_perf_changes: allowed changes in stats. This is a dictionary as returned by get_allowed_perf_changes().
# perf_stat: The PerfStat of the test.
# change: the MetricChange to check.
# Returns True if the change is allowed, False otherwise.
def is_perf_change_allowed(allowed_perf_changes, perf_stat, change):
    allowed_change_directions =  [MetricChange.NoChange] + [ allow_stmt['direction']
            for allow_stmt in allowed_perf_changes.get(perf_stat.test, [])

            # List of metrics are not specified or the metric is in the list of metrics.
            if not allow_stmt['metrics'] or perf_stat.metric in allow_stmt['metrics']

            # way/test are not specified, or match the actual way/test.
            if ((not 'way'      in allow_stmt['opts'].keys()) or perf_stat.way      == allow_stmt['opts']['way'])
            if ((not 'test_env' in allow_stmt['opts'].keys()) or perf_stat.test_env == allow_stmt['opts']['test_env'])
        ]
    return change in allowed_change_directions


# Calculates a suggested string to append to the git commit in order to accept the
# given changes.
# changes: [(MetricChange, PerfStat)]
# Returns a suggested git commit string.
...
def check_stats_change(actual, expected_val, tolerance_dev, allowed_perf_changes = {}, force_print = False):
    ...
    allowed_change_directions = is_perf_change_allowed(allowed_perf_changes, actual, change)
